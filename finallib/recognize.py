"""
nzk/face/recognize.py — face recognition against enrolled embeddings
...
confidence via known=False.

usage:
    from nzk.face.recognize import Recognizer

    with Recognizer() as rec:
        results = rec.process(rgb, depth, ir, bodies)
        for r in results:
            print(r.name, r.confidence, r.known)
            print(r.body)   # BodyResult | None
"""
...
from ..types import FaceResult, BodyResult, Vec3
from .._ir import detection_image
...
class Recognizer:
    ...
    def process(
        ...
        bodies:      list[BodyResult]  = None,
    ) -> list[FaceResult]:
        """
        rgb_frame:   (1080, 1920, 4) uint8  BGRX
        depth_frame: (424, 512)      float32 mm
        ir_frame:    (424, 512)      float32  — optional
        bodies:      list of BodyResult from BodyTracker (for skeleton assignment)
                     pass None or [] to skip skeleton association.

        returns list of FaceResult, one per detected face.
        """
        detect_img, source = detection_image(rgb_frame, ir_frame)
